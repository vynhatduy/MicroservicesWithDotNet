            ...
            if (lbl > 0) {
                ...
                if (token !== null && token !== "null" && token !== "" & token !== undefined &&decodeToken!==null) {
                    ...
                    if (existingProductIndex !== -1) {
                        cartData[existingProductIndex].soLuong += lbl;
                        cartData[existingProductIndex].gia += gia;
                        window.alert("Đã thêm vào giỏ hàng");
                        window.location.reload();
                    } else {
                        const data = {
                ...
                <div className="info">
                {
                    list.map((item) => (
                        <div className="row" key={item.id}>
                            ...
                            <div className="col-7">
                                <h1 id="ten">{item.ten}</h1>
                                <span>{item.moTa}</span>
                                {
                                    cate.map((itemData) => (
                                        <div className="heading" key={itemData.key}>
                                            <span>Số lượng : {item.soluong}    |    Đã bán : {item.daban}</span>
                                        </div>
                                    ))
                                }
                                <h2 id="gia">{Formater(item.gia)}</h2>
                                ...
                                <div className="btn-addtocart">
                                    <button id="add-to-cart" className="add-cart" onClick={() => addToCart(item.id,item.url, item.ten, item.gia)} ><MdAddShoppingCart /> Thêm vào giỏ hàng</button>
